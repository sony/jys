...
class ImageNet_class_conditional_generator():
    def checkpoint_canonical_path(maskgit_or_tokenizer, image_size, ckpt_path=None):
        if ckpt_path is None:
            return f"./checkpoints/{maskgit_or_tokenizer}_imagenet{image_size}_checkpoint"
        else:
            return f"{ckpt_path}/{maskgit_or_tokenizer}_imagenet{image_size}_checkpoint"

    def __init__(self, image_size=256, ckpt_path=None):
        ...
        self._load_checkpoints(ckpt_path)
    
    def __call__(self, indices, label=None, *args, **kwargs):
        '''jax model wrapper'''
        if label is None:
            label = torch.randnint(0, 1000, (indices.shape[0], )).to(indices)
        # torch.Tensor -> jax.numpy.array
        if isinstance(indices, torch.Tensor):
            dl = asdlpack(indices)
            indices = jax.numpy.from_dlpack(dl)
        if isinstance(label, torch.Tensor):
            dl = asdlpack(label)
            label = jax.numpy.from_dlpack(dl)
        
        B = indices.shape[0]
        C = self.maskgit_cf.vqvae.codebook_size
        # Create label_tokens
        label_tokens = label[:, None] * jnp.ones([B, 1])
        label_tokens = label_tokens + C
        # Concatenate the two as input_tokens
        indices = jnp.concatenate([label_tokens, indices], axis=-1)
        indices = indices.astype(jnp.int32)
        # Model forward
        logits = self.transformer_model.apply(self.transformer_variables, indices, deterministic=True)
        logits = logits[:, 1:, :]
        logits = jnp.take(logits, jnp.array(list(range(C))+[-1]), axis=-1)
        
        # jax.numpy.array -> torch.Tensor
        if not isinstance(logits, torch.Tensor):
            dl = asdlpack(logits)
            logits = torch.from_dlpack(dl)
        logits[..., :C] = logits[..., :C].softmax(dim=-1)
        return logits
    
    def _load_checkpoints(self, ckpt_path):
        image_size = self.maskgit_cf.image_size

        self.transformer_variables = restore_from_path(
            ImageNet_class_conditional_generator.checkpoint_canonical_path("maskgit", image_size, ckpt_path))
        self.tokenizer_variables = restore_from_path(
            ImageNet_class_conditional_generator.checkpoint_canonical_path("tokenizer", image_size, ckpt_path))
    ...
    def create_input_tokens_normal(self, label: int, eval_batch_size=None):
        label_tokens = label * jnp.ones([eval_batch_size, 1])
        # Shift the label by codebook_size 
        label_tokens = label_tokens + self.maskgit_cf.vqvae.codebook_size
        # Create blank masked tokens
        blank_tokens = jnp.ones([eval_batch_size, self.transformer_block_size-1])
